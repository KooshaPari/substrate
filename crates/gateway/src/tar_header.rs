...
mod tests {
    ...
    #[test]
    fn name_roundtrip() {
        let b = make_name("test.sh");
        assert_eq!(parse_name(&b), Some("test.sh".into()));
    }
    #[test]
    fn name_empty() {
        let b = [0u8; 100];
        assert_eq!(parse_name(&b), Some("".into()));
    }
    #[test]
    fn octal() {
        let mut b = [0u8; 12];
        b[..5].copy_from_slice(b"00065");
        assert_eq!(parse_octal(&b), Some(0o65));
    }
    #[test]
    fn octal_zero() {
    ...
    fn long_name_truncates() {
        ...
        let b = make_name(&name);
        assert_eq!(parse_name(&b), Some("x".repeat(99)));
    }
}
