...
impl<K: Hash + Eq + Clone, V> ShardMap2<K, V> {
    ...
    pub fn insert(&mut self, k: K, v: V) -> Option<V> { self.shards[self.shard_idx(&k)].insert(k, v) }
    pub fn get<Q>(&self, k: &Q) -> Option<&V> where K: std::borrow::Borrow<Q>, Q: Hash + Eq + ?Sized { self.shards[self.shard_idx(k)].get(k) }
    pub fn remove<Q>(&mut self, k: &Q) -> Option<V> where K: std::borrow::Borrow<Q>, Q: Hash + Eq + ?Sized { self.shards[self.shard_idx(k)].remove(k) }
