...
pub fn parse_field(s: &str, lo: u32, hi: u32) -> Result<Field, String> {
    if s == "*" { return Ok(Field::All); }
    if let Some(rest) = s.strip_prefix("*/") {
        let step: u32 = rest.parse().map_err(|_| format!("bad step: {}", s))?;
        if step == 0 { return Err("step must be > 0".into()); }
    ...
    if s.contains(',') {
        ...
        for p in s.split(',') {
            match parse_field(p, lo, hi)? {
                ...
                Field::Range(a, b) => for v in a..=b { out.push(v); },
                Field::Step(a, step) => { let mut v = a; while v <= hi { out.push(v); v += step; } },
    ...
    if let Some(dash) = s.find('-') {
        let a: u32 = s[..dash].parse().map_err(|_| format!("bad range start: {}", s))?;
        let b: u32 = s[dash + 1..].parse().map_err(|_| format!("bad range end: {}", s))?;
        if a < lo || b > hi || a > b { return Err(format!("range out of bounds: {}", s)); }
        return Ok(Field::Range(a, b));
    }
    let n: u32 = s.parse().map_err(|_| format!("bad number: {}", s))?;
    if n < lo || n > hi { return Err(format!("out of bounds: {}", n)); }
    Ok(Field::Specific(n))
}

pub fn matches(c: &Cron, ts: (u32, u32, u32, u32, u32)) -> bool {
    let (m, h, d, mo, w) = ts;
    in_field(&c.minute, m) && in_field(&c.hour, h) && in_field(&c.day, d)
        && in_field(&c.month, mo) && in_field(&c.weekday, w)
}
...
mod tests {
    use super::*;
    #[test] fn parse_5_fields() {
        assert!(parse("0 0 * * *").is_ok());
        assert!(parse("* * * * * *").is_err());
    }
    #[test] fn parse_specific() {
