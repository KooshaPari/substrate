...
mod tests {
    use super::*;
    #[test] fn equinox() { let (s, e) = sunrise_sunset(2024, 3, 20, 0.0, 0.0); assert!(s < e); assert!((s - 6.0).abs() < 0.5); }
    #[test] fn summer() { let (s, e) = sunrise_sunset(2024, 6, 21, 40.0, -74.0); assert!(s < 5.5); assert!(e > 20.0); }
    #[test] fn winter() { let (s, e) = sunrise_sunset(2024, 12, 21, 40.0, -74.0); assert!(s > 7.0); assert!(e < 17.0); }
}
