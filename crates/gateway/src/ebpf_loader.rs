...
pub fn parse_elf(input: &[u8]) -> Result<(ElfHeader, Vec<Section>), String> {
    ...
    let read_u64 = |off: usize| -> u64 {
        if is_le {
            ...
            u64::from_be_bytes(input[off..off + 8].try_into().unwrap())
        }
    };
    let (
        e_type_off,
    ...
    let e_entry = if is_64 {
        read_u64(e_entry_off)
    } else {
        read_u32(e_entry_off) as u64
    };
    let e_phoff = if is_64 {
        read_u64(e_phoff_off)
    } else {
        read_u32(e_phoff_off) as u64
    };
    let e_shoff = if is_64 {
        read_u64(e_shoff_off)
    } else {
        read_u32(e_shoff_off) as u64
    };
    ...
    for i in 0..e_shnum as usize {
        ...
        if off + shentsize > input.len() {
            return Err(format!("section header {} past EOF", i));
        }
        let s = Section {
            ...
            sh_flags: if is_64 {
                ...
                read_u32(off + 8) as u64
            },
            sh_addr: if is_64 {
                read_u64(off + 16)
            } else {
                read_u32(off + 12) as u64
            },
            sh_offset: if is_64 {
                read_u64(off + 24)
            } else {
                read_u32(off + 16) as u64
            },
            sh_size: if is_64 {
                read_u64(off + 32)
            } else {
                read_u32(off + 20) as u64
            },
            sh_link: read_u32(if is_64 { off + 40 } else { off + 24 }),
            sh_info: read_u32(if is_64 { off + 44 } else { off + 28 }),
            sh_addralign: if is_64 {
                read_u64(off + 48)
            } else {
                read_u32(off + 32) as u64
            },
            sh_entsize: if is_64 {
                read_u64(off + 56)
            } else {
                read_u32(off + 36) as u64
            },
            data: Vec::new(),
        };
        sections.push(s);
...
pub fn parse_symbols(symtab: &Section) -> Result<Vec<Symbol>, String> {
    if symtab.sh_type != SHT_SYMTAB {
        return Err("not a symbol table".into());
    }
    if symtab.data.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn find_section<'a>(
    ...
    sections
        .iter()
        .find(|s| read_strtab(shstrtab, s.name_offset) == name)
}
...
pub fn extract_programs(
    ...
    for sec in sections {
        ...
        let license = if let Some(lic_sec) = sections
            .iter()
            .find(|s| read_strtab(shstrtab, s.name_offset) == "license")
        {
            if let Ok(s) = std::str::from_utf8(&lic_sec.data) {
            ...
            for sym in &syms {
                ...
                if sym.info >> 4 == 2 && sym.size > 0 {
                    for prog in out.iter_mut() {
                        if prog.name == format!(".text")
                            || sym_name == prog.name.trim_start_matches('.')
                        {
...
mod tests {
    ...
    fn find_section_empty() {
        ...
        assert_eq!(
            find_section(&[text], &shstrtab, ".text").unwrap().sh_type,
            SHT_PROGBITS
        );
        assert!(find_section(&[], &shstrtab, "nope").is_none());
    }
    #[test]
    fn read_strtab_works() {
        let sec = Section {
            name_offset: 0,
            sh_type: SHT_STRTAB,
            sh_flags: 0,
    ...
    #[test]
    fn extract_no_bpf_returns_empty() {
