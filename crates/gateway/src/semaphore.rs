...
impl Drop for SemaphoreGuard {
    fn drop(&mut self) {
        self.sem.lock().unwrap().available += 1;
    }
}
...
mod tests {
    ...
    #[test]
    fn weighted_acquire_release() {
        let s = WeightedSemaphore::new(10);
        assert!(s.try_acquire(7));
    ...
    #[test]
    fn weighted_release_clamps() {
        let s = WeightedSemaphore::new(10);
        s.release(100);
        assert!(s.try_acquire(10));
    }
}
