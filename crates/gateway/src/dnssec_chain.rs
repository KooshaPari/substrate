...
pub fn parse_dnskey_wire(rdata: &[u8]) -> Result<DnskeyRdata, String> {
    if rdata.len() < 4 {
        return Err("dnskey rdata too short".into());
    }
...
pub fn parse_ds_wire(rdata: &[u8]) -> Result<DsRdata, String> {
    if rdata.len() < 4 {
        return Err("ds rdata too short".into());
    }
...
pub fn verify_ds_to_dnskey(ds: &DsRdata, dnskey_rdata: &[u8]) -> Result<bool, String> {
    ...
    if dnskey_rdata.len() < 4 || ds.algorithm != dnskey_rdata[3] {
        return Ok(false);
    }
...
pub fn validate_chain(
    parent_dnskeys: &[(u16, Vec<u8>)],
    child_ds_records: &[DsRdata],
) -> Vec<ChainLink> {
...
mod tests {
    ...
    #[test]
    fn chain_validate_empty() {
        let links = validate_chain(&[], &[]);
        assert!(links.is_empty());
    }
    #[test]
    fn chain_validate_match() {
    ...
    #[test]
    fn chain_validate_mismatch() {
        let dnskey = vec![0x01, 0x00, 0x03, 0x0d, 0xaa];
        let kt = compute_key_tag(&dnskey);
        let bogus_ds = DsRdata {
            key_tag: kt,
            algorithm: 0x0d,
            digest_type: 2,
            digest: vec![0; 32],
