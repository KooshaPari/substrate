...
pub fn encode(v: &Value, out: &mut Vec<u8>) {
    match v {
        ...
        Value::Unknown(_, _) => { out.push(0xff); }
    }
}

fn encode_head(major: u8, arg: u64, out: &mut Vec<u8>) {
    let lead = major << 5;
    if arg < 24 { out.push(lead | arg as u8); }
    else if arg < 0x100 {
        out.push(lead | 24);
        out.push(arg as u8);
    } else if arg < 0x10000 {
...
pub fn decode(input: &[u8]) -> Result<(Value, usize), String> {
    ...
    match major {
        ...
        2 => {
            if input.len() < head_size + arg as usize { return Err("bstr truncated".into()); }
            let bytes = input[head_size..head_size + arg as usize].to_vec();
            Ok((Value::Bytes(bytes), head_size + arg as usize))
        }
        3 => {
            if input.len() < head_size + arg as usize { return Err("tstr truncated".into()); }
            let raw = &input[head_size..head_size + arg as usize];
            let s = std::str::from_utf8(raw).map_err(|_| "bad utf8")?.to_string();
...
fn read_arg(info: u8, input: &[u8]) -> Result<(u64, usize), String> {
    match info {
        ...
        24 => {
            if input.len() < 2 { return Err("u8 truncated".into()); }
            Ok((input[1] as u64, 2))
        }
        25 => {
            if input.len() < 3 { return Err("u16 truncated".into()); }
            Ok((u16::from_be_bytes([input[1], input[2]]) as u64, 3))
        }
        26 => {
            if input.len() < 5 { return Err("u32 truncated".into()); }
            Ok((u32::from_be_bytes([input[1], input[2], input[3], input[4]]) as u64, 5))
        }
        27 => {
            if input.len() < 9 { return Err("u64 truncated".into()); }
...
mod tests {
    ...
    #[test] fn small_uint() { assert_eq!(rt(&Value::UInt(5)), Value::UInt(5)); }
    #[test] fn large_uint() { assert_eq!(rt(&Value::UInt(1_000_000)), Value::UInt(1_000_000)); }
    #[test] fn huge_uint() { assert_eq!(rt(&Value::UInt(u64::MAX)), Value::UInt(u64::MAX)); }
    #[test] fn nint() { assert_eq!(rt(&Value::NInt(-1)), Value::NInt(-1)); assert_eq!(rt(&Value::NInt(-1000)), Value::NInt(-1000)); }
    #[test] fn text() { assert_eq!(rt(&Value::Text("hello".into())), Value::Text("hello".into())); }
    #[test] fn long_text() {
        let s = "x".repeat(1000);
    ...
    #[test] fn empty_array() { assert_eq!(rt(&Value::Array(vec![])), Value::Array(vec![])); }
    #[test] fn empty_map() { assert_eq!(rt(&Value::Map(vec![])), Value::Map(vec![])); }
    #[test] fn rejects_indefinite() {
        let buf = vec![0x1f];
        assert!(decode(&buf).is_err());
    }
    #[test] fn rejects_truncated() {
        let buf = vec![0x19];
        assert!(decode(&buf).is_err());
    }
}