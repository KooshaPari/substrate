...
pub fn parse_header(data: &[u8]) -> Result<(GzipHeader, usize), GzipParseError> {
    if data.len() < 10 { return Err(GzipParseError::TooShort); }
    if data[0] != MAGIC[0] || data[1] != MAGIC[1] { return Err(GzipParseError::BadMagic); }
    let cm = data[2];
    if cm != 8 { return Err(GzipParseError::UnknownCompression); }
    let flg = data[3];
    ...
    let extra = if flg & 0x04 != 0 {
        if pos + 2 > data.len() { return Err(GzipParseError::TooShort); }
        let xlen = u16::from_le_bytes([data[pos], data[pos + 1]]) as usize;
        pos += 2;
        if pos + xlen > data.len() { return Err(GzipParseError::TooShort); }
    ...
    let name = if flg & 0x08 != 0 {
        ...
        if pos >= data.len() { return Err(GzipParseError::TooShort); }
        let s = std::str::from_utf8(&data[start..pos]).map_err(|_| GzipParseError::TooShort)?.to_string();
        pos += 1;
        Some(s)
    } else { None };
    ...
    Ok((GzipHeader { mtime, xfl, os, extra, name, comment }, pos))
}
...
mod tests {
    ...
    #[test] fn unknown_compression() {
        let v = [0x1f, 0x8b, 9, 0, 0, 0, 0, 0, 0, 0];
        assert!(matches!(parse_header(&v), Err(GzipParseError::UnknownCompression)));
    }
    #[test] fn with_name() {
