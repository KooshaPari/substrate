                        ...
                        let (status, pr_urls) = match structured {
                            ...
                            Some(ref r) => {
                                let _ = store.task_update(
                                    ...
                                    Some(&format!("non-terminal engine status: {:?}", r.status)),
                                );
                                (
                                    LaneStatus::Failed(format!(
                                        "engine returned non-terminal status: {:?}",
                                        r.status
                                    )),
                                    r.pr_urls.clone(),
                                )
                            ...
                            None => {
                                let _ = store.task_update(
                                    lane_task_id,
                                    MailboxTaskState::Failed,
                                    Some("dump/extract failed"),
                                );
                                (
                                    LaneStatus::Failed("failed to extract engine result".into()),
                                    vec![],
                                )
                            }
                        };
