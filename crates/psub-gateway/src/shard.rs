...
pub struct ShardMap<K: Hash + Eq, V> { shards: Vec<HashMap<K, V>>, shard_count: usize }
impl<K: Hash + Eq, V> ShardMap<K, V> {
    pub fn new(shard_count: usize) -> Self {
        let n = shard_count.max(1);
        assert!(n > 0);
        let mut shards = Vec::with_capacity(n); for _ in 0..n { shards.push(HashMap::new()); }
        Self { shards, shard_count: n }
    }
    fn idx<Q: Hash + ?Sized>(&self, k: &Q) -> usize {
    ...
    pub fn get<Q>(&self, k: &Q) -> Option<&V> where K: std::borrow::Borrow<Q>, Q: Hash + Eq + ?Sized {
        let idx = self.idx(k);
        self.shards[idx].get(k)
    }
    pub fn remove<Q>(&mut self, k: &Q) -> Option<V> where K: std::borrow::Borrow<Q>, Q: Hash + Eq + ?Sized {
        let idx = self.idx(k);
