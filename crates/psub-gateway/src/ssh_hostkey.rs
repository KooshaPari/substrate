...
pub fn parse(input: &[u8]) -> Result<HostKey, String> {
    ...
    match key_type_str {
        "ssh-ed25519" => {
            ...
            if key_bytes.len() != 32 { return Err("ed25519 key must be 32 bytes".into()); }
            let mut key = [0u8; 32];
        ...
        "ssh-rsa" => {
            ...
            Ok(HostKey::Rsa { e: e.to_vec(), n: n.to_vec() })
        }
        "ecdsa-sha2-nistp256" => {
            let (curve, rest) = read_string(rest)?;
            let (q, _) = read_string(rest)?;
            Ok(HostKey::EcdsaNistp256 {
                curve: std::str::from_utf8(curve).map_err(|_| "bad curve name")?.to_string(),
                q: q.to_vec(),
            })
        }
        other => Ok(HostKey::Unknown { key_type: other.to_string(), blob: rest.to_vec() }),
    }
}

fn read_string(input: &[u8]) -> Result<(&[u8], &[u8]), String> {
    if input.len() < 4 { return Err("string length truncated".into()); }
    let len = u32::from_be_bytes([input[0], input[1], input[2], input[3]]) as usize;
    if input.len() < 4 + len { return Err("string value truncated".into()); }
...
pub fn key_metadata(key: &HostKey) -> BTreeMap<String, String> {
    ...
    match key {
        HostKey::Ed25519 { .. } => { m.insert("type".into(), "ssh-ed25519".into()); }
        HostKey::Rsa { .. } => { m.insert("type".into(), "ssh-rsa".into()); }
        HostKey::EcdsaNistp256 { curve, .. } => { m.insert("type".into(), "ecdsa-sha2".into()); m.insert("curve".into(), curve.clone()); }
        HostKey::Unknown { key_type, .. } => { m.insert("type".into(), key_type.clone()); }
    }
    m
}
