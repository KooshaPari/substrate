...
pub fn unpad(data: &[u8], block_size: usize) -> Result<&[u8], String> {
    ...
    if pad_len > data.len() { return Err("pad exceeds data".into()); }
    for &b in &data[data.len() - pad_len..] {
        if b != pad_byte { return Err("inconsistent pad".into()); }
    }
...
mod tests {
    ...
    #[test] fn pad_empty_full_block() {
        let p = pad(&[], 8);
        assert_eq!(p, vec![8u8; 8]);
    }
    #[test] fn pad_empty_aes() {
        let p = pad(&[], 16);
        assert_eq!(p, vec![16u8; 16]);
    }
    #[test] fn round_trip_various_sizes() {
        for size in 0..50 {
    ...
    #[test] fn unpad_exact_block() {
        let p = pad(b"01234567", 8);
        assert_eq!(unpad(&p, 8).unwrap(), b"01234567");
    }
    #[test] fn unpad_partial() {
        let p = pad(b"hello", 8);
    ...
    #[test] fn unpad_bad_alignment() {
        assert!(unpad(b"hello", 8).is_err());
    }
    #[test] fn unpad_bad_pad_byte() {
        let bad = vec![0u8; 8];
        assert!(unpad(&bad, 8).is_err());
    }
    #[test] fn unpad_pad_exceeds_block() {
        let bad = vec![20u8; 8];
        assert!(unpad(&bad, 8).is_err());
    }
    #[test] fn unpad_inconsistent() {
