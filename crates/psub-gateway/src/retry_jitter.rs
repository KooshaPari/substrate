...
pub fn exponential_backoff_jitter_ms(attempt: u32, base_ms: u64, max_ms: u64, jitter_pct: f64) -> u64 {
...
mod tests {
    ...
    #[test] fn backoff_capped() {
        assert_eq!(exponential_backoff_ms(50, 100, 1000), 1000);
    }
    #[test] fn backoff_attempt_zero_safe() {
        assert!(exponential_backoff_ms(0, 50, 10_000) >= 50);
    }
    #[test] fn jitter_within_bounds() {
        for _ in 0..20 {
            let v = exponential_backoff_jitter_ms(3, 100, 60_000, 0.5);
            assert!(v >= 100 && v <= 1500, "jitter out of bounds: {}", v);
        }
    }
    #[test] fn retry_on_429() { assert!(should_retry(429, 0, 5)); }
    #[test] fn retry_on_500() { assert!(should_retry(500, 1, 5)); }
    #[test] fn no_retry_on_400() { assert!(!should_retry(400, 0, 5)); }
    #[test] fn no_retry_after_max() { assert!(!should_retry(500, 5, 5)); }
    #[test] fn jitter_pct_clamped() {
