...
//!
//! [`Backoff`] is the core type: a configured retry schedule that maps an
//! attempt index (zero-based) to a [`Duration`] delay. Three strategies are
//! supported via [`BackoffStrategy`]:
//!
//! - [`Fixed`](BackoffStrategy::Fixed): constant delay regardless of attempt.
//! - [`Linear`](BackoffStrategy::Linear): delay grows linearly with attempt.
//! - [`Exponential`](BackoffStrategy::Exponential): delay doubles per attempt,
//!   capped at `max`.
//!
//! Useful inside the gateway for provider retries — wrap a `Backoff::new(...)`
//! around an upstream call loop and call [`Backoff::delay_for`] with each attempt.
...
mod tests {
    ...
    fn base_and_max_accessors() {
        ...
        assert_eq!(b.max(), Duration::from_millis(750));
    }
}