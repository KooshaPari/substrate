...
const K0: [u32; 5] = [0x0000_0000, 0x5A82_7999, 0x6ED9_EBA1, 0x8F1B_BCDC, 0xA953_FD4E];
const K1: [u32; 5] = [0x50A2_8BE6, 0x5C4D_D124, 0x6D70_3EF3, 0x7A6D_76E9, 0x0000_0000];
...
fn process_block(state: &mut [u32; 5], block: &[u8; 64]) {
    ...
    for j in 0..80 {
        let t = a.wrapping_add(f(j, b, c, d))
            .wrapping_add(x[R1[j]])
