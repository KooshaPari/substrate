...
fn match_recursive(p: &[char], n: &[char]) -> bool {
    ...
    match p[0] {
        '*' => {
            ...
            for i in 0..=n.len() { if match_recursive(&p[1..], &n[i..]) { return true; } }
            false
        }
...
mod tests {
    ...
    #[test] fn star() { assert!(matches("a*", "abc")); assert!(matches("*", "anything")); assert!(matches("*x", "abcx")); }
    #[test] fn question() { assert!(matches("a?c", "abc")); assert!(!matches("a?c", "ac")); }
    #[test] fn empty() { assert!(matches("", "")); assert!(!matches("", "x")); assert!(!matches("x", "")); }
    #[test] fn combined() { assert!(matches("a*b?c", "axxxbyc")); assert!(!matches("a*b?c", "axxxb")); }
}
