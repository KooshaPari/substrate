...
pub fn parse(input: &str) -> Vec<M3uEntry> {
    let mut out = Vec::new();
    let mut current = M3uEntry { duration_secs: None, title: None, uri: String::new() };
...
pub fn render(entries: &[M3uEntry]) -> String {
    ...
    out.push_str("#EXTM3U\n");
    for e in entries {
        let dur = e.duration_secs.map(|d| format!("{}", d)).unwrap_or_else(|| "-1".into());
...
mod tests {
    ...
    #[test] fn parse_no_extm3u_still_works() {
        let m = parse("#EXTINF:120,Hello\nhttp://example.com/stream.mp3\n");
        assert_eq!(m.len(), 1);
        assert_eq!(m[0].title.as_deref(), Some("Hello"));
    }
    #[test] fn parse_no_extinf() {
        let m = parse("#EXTM3U\nhttp://example.com/stream.mp3\n");
        assert_eq!(m.len(), 1);
        assert_eq!(m[0].uri, "http://example.com/stream.mp3");
    ...
    #[test] fn parse_comments_ignored() {
        let m = parse("#EXTM3U\n# some comment\n#EXTINF:60,A\nhttp://a\n");
        assert_eq!(m.len(), 1);
    }
    #[test] fn parse_fractional_duration() {
        let m = parse("#EXTM3U\n#EXTINF:120.5,A\nhttp://a\n");
    ...
    #[test] fn render_roundtrip() {
        let entries = vec![
            M3uEntry { duration_secs: Some(60.0), title: Some("A".into()), uri: "http://a".into() },
            M3uEntry { duration_secs: None, title: None, uri: "http://b".into() },
        ];
