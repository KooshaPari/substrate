...
pub enum Selection {
    ...
    InlineFragment { on_type: Option<String>, selection: Vec<Selection> },
}
...
impl<'a> Parser<'a> {
    ...
    fn skip_ws_and_comments(&mut self) {
        while let Some(c) = self.peek() {
            ...
            else if c == '#' { while let Some(c) = self.peek() { if c == '\n' { break; } self.advance(); } }
            else { break; }
        }
    }
    fn expect(&mut self, ch: char) -> Result<(), String> {
        ...
        else { Err(format!("expected '{}', got {:?}", ch, self.peek())) }
    }
    ...
    fn parse_identifier(&mut self) -> Result<String, String> {
        ...
        if self.pos == start { return Err(format!("expected identifier pos={} (before_loop={}) rest={:?}", self.pos, before_loop, &self.src[before_loop..self.src.len().min(before_loop+10)])); }
    ...
    fn parse_definition(&mut self) -> Result<Definition, String> {
        ...
        if self.try_keyword("fragment") {
            ...
            self.expect('}')?;
            Ok(Definition::Fragment(Fragment { name, on_type, selection }))
        } else if self.peek_keyword("query") || self.peek_keyword("mutation") || self.peek_keyword("subscription") {
    ...
    fn peek_keyword(&self, kw: &str) -> bool {
        ...
        let after = self.pos + kw.len();
        after >= self.src.len() || matches!(self.src.as_bytes()[after], b' ' | b'\t' | b'\n' | b'\r' | b'(' | b'{' | b',' | b'}')
    }
    fn parse_operation(&mut self) -> Result<Operation, String> {
        let kind = if self.try_keyword("query") { OperationKind::Query }
        else if self.try_keyword("mutation") { OperationKind::Mutation }
        else if self.try_keyword("subscription") { OperationKind::Subscription }
        else { return Err("expected operation type".into()); };
        self.skip_ws_and_comments();
        let name = if self.peek() == Some('{') || self.peek() == Some('(') { None } else { Some(self.parse_identifier()?) };
        let vars = if self.peek() == Some('(') { self.parse_var_defs()? } else { Vec::new() };
        self.expect('{')?;
        let selection = self.parse_selection_set()?;
        self.expect('}')?;
        Ok(Operation { kind, name, vars, selection })
    }
    fn try_keyword(&mut self, kw: &str) -> bool {
        ...
        let boundary_ok = after >= self.src.len()
            || matches!(self.src.as_bytes()[after], b' ' | b'\t' | b'\n' | b'\r' | b'(' | b'{' | b',' | b'}');
        if boundary_ok {
            self.pos = after;
            true
        } else { false }
    ...
    fn parse_var_defs(&mut self) -> Result<Vec<VarDef>, String> {
        ...
        loop {
            self.skip_ws_and_comments();
            if self.peek() == Some(')') { self.advance(); return Ok(out); }
            if self.advance() != Some('$') { return Err(format!("expected $ at pos={}", self.pos)); }
            let name = self.parse_identifier()?;
            self.expect(':')?;
            let ty = self.parse_type_ref()?;
            let default = if self.peek() == Some('=') { self.advance(); Some(self.parse_value()?) } else { None };
            out.push(VarDef { name, ty, default });
        }
    }
    fn parse_type_ref(&mut self) -> Result<String, String> {
        let name = self.parse_identifier()?;
        if self.peek() == Some('!') { self.advance(); Ok(name + "!") }
        else { Ok(name) }
    }
    ...
    fn parse_selection(&mut self) -> Result<Selection, String> {
        ...
        if self.src[self.pos..].starts_with("...") {
            ...
            if self.peek() == Some('o') && self.src[self.pos..].starts_with("on ") {
                ...
                self.expect('}')?;
                return Ok(Selection::InlineFragment { on_type: Some(on_type), selection: sel });
            }
            let name = self.parse_identifier()?;
        ...
        let args = if self.peek() == Some('(') { self.parse_args()? } else { Vec::new() };
        self.skip_ws_and_comments();
        ...
        Ok(Selection::Field(Field { alias, name: first, args, selection }))
    }
    ...
    fn parse_value(&mut self) -> Result<Value, String> {
        ...
        match c {
            '$' => { self.advance(); let name = self.parse_identifier()?; Ok(Value::String(format!("${}", name))) }
            '[' => { self.advance(); let mut items = Vec::new(); loop { self.skip_ws_and_comments(); if self.peek() == Some(']') { self.advance(); return Ok(Value::List(items)); } items.push(self.parse_value()?); self.skip_ws_and_comments(); if self.peek() == Some(',') { self.advance(); } } }
            '{' => { self.advance(); let mut entries = Vec::new(); loop { self.skip_ws_and_comments(); if self.peek() == Some('}') { self.advance(); return Ok(Value::Object(entries)); } let k = self.parse_identifier()?; self.expect(':'); let v = self.parse_value()?; entries.push((k, v)); self.skip_ws_and_comments(); if self.peek() == Some(',') { self.advance(); } } }
            '"' => Ok(Value::String(self.parse_string()?)),
            '-' | '0'..='9' => self.parse_number(),
            't' | 'f' => {
                if self.src[self.pos..].starts_with("true") { self.pos += 4; Ok(Value::Bool(true)) }
                else if self.src[self.pos..].starts_with("false") { self.pos += 5; Ok(Value::Bool(false)) }
                else { Err("expected true/false".into()) }
            }
            'n' => { if self.src[self.pos..].starts_with("null") { self.pos += 4; Ok(Value::Null) } else { Err("expected null".into()) } }
            _ => {
                let name = self.parse_identifier()?;
                Ok(Value::Enum(name))
            }
    ...
    fn parse_string(&mut self) -> Result<String, String> {
        if self.advance() != Some('"') { return Err("expected '\"'".into()); }
...
mod tests {
    ...
    #[test] fn mutation_with_vars() {
        let defs = parse("mutation AddPost($title: String!) { addPost(title: $title) { id } }").unwrap();
        if let Definition::Operation(op) = &defs[0] {
    ...
    #[test] fn args_object_value() {
        let defs = parse("{ user(filter: { active: true, tags: [\"a\", \"b\"] }) { id } }").unwrap();
        if let Definition::Operation(op) = &defs[0] {
            if let Selection::Field(f) = &op.selection[0] {
    ...
    #[test] fn fragment_spread() {
        let defs = parse("query { user { ...UserFields } } fragment UserFields on User { id name }").unwrap();
