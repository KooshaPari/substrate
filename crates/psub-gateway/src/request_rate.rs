...
mod tests {
    ...
    fn empty_rate_is_zero() {
        assert_eq!(RequestRate::new().rate_per_sec(), 0.0);
    }
    #[test]
    fn record_increments_count() {
        let r = RequestRate::new();
        r.record();
        r.record();
        assert_eq!(r.count_in_window(), 2);
    }
    #[test]
    fn rate_reflects_count() {
        let r = RequestRate::new();
        for _ in 0..10 {
            r.record();
        }
    ...
    fn clone_shares_state() {
        ...
        let r2 = r.clone();
        r.record();
        assert_eq!(r2.count_in_window(), 1);
    }
    #[test]
    fn default_is_empty() {
        assert_eq!(RequestRate::default().count_in_window(), 0);
    }
    #[test]
    fn high_volume_bounded() {
        let r = RequestRate::new();
        for _ in 0..1000 {
            r.record();
        }
        assert_eq!(r.count_in_window(), 1000);
    }
}
