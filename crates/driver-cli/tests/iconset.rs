...
fn brand_svg_uses_backbone2_palette() {
    let content = std::fs::read_to_string(brand_dir().join("substrate-icon.svg")).expect("read svg");
    let palette = [
...
fn driver_cli_cargo_toml_has_bundle_metadata_block() {
    ...
    assert!(toml.contains("substrate.iconset"), "bundle.icon must reference substrate.iconset");
